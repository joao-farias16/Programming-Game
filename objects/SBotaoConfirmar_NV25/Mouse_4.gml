if (instance_exists(SAreaResposta_NV25)) {
    
    // Se o jogador já acertou e clicar novamente,
    // avança para a tela de níveis
    if (SAreaResposta_NV25.estado_atual == ESTADO_FASE.ACERTO) {
        global.fase25_concluida = true;
    ...
    // Se ainda está testando o código:
        ...
        var comando = scr_normalizar_codigo(SAreaResposta_NV25.texto_digitado);
        
        // Respostas aceitas (variações válidas do código corrigido)
        var resposta_1 = scr_normalizar_codigo("x = 0\nwhile x < 5:\nprint(x)\nx += 1");
        ...
        if (comando == resposta_1) {
			...
			scr_pontos_errar(SAreaResposta_NV25, 100);
			...
			with (LampadaNivel25)
			{
				...
				if (contador_erros >= 2 && nivel_dica < 1)
				{
					...
					scr_pontos_errar(SAreaResposta_NV25, 200);
				}
				...
				else if (contador_erros >= 4 && nivel_dica < 2)
				{
					...
					scr_pontos_errar(SAreaResposta_NV25, 300);
				}
				...
				else if (contador_erros >= 6 && nivel_dica < 3)
				{
					...
					scr_pontos_errar(SAreaResposta_NV25, 400);
				}
			}
            ...
            SAreaResposta_NV25.alarm[0] = 1 * game_get_speed(gamespeed_fps);
        } 
    } 
}