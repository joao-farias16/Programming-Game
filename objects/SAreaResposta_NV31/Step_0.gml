...
if (estado_atual == ESTADO_FASE.ACERTO) {
    ...
    if (keyboard_check_pressed(vk_return)) {
        ...
        if (comando == "in") {
			...
			scr_pontos_confirmar_acerto(SAreaResposta_NV31, "");

        } else {
            estado_atual = ESTADO_FASE.ERRO;
			scr_pontos_errar(SAreaResposta_NV31, 100);
			...
			with (LampadaNivel31)
			{
				...
				if (contador_erros >= 2 && nivel_dica < 1)
				{
					...
					scr_pontos_errar(SAreaResposta_NV31, 200);
				}
				...
				else if (contador_erros >= 4 && nivel_dica < 2)
				{
					...
					scr_pontos_errar(SAreaResposta_NV31, 300);
				}
				...
				else if (contador_erros >= 6 && nivel_dica < 3)
				{
					...
					scr_pontos_errar(SAreaResposta_NV31, 400);
				}
			}
